# memb.py  |  Telegram Channel Membership Checker (fixed version)
# Works standalone or when executed remotely with exec()
# Auto-saves user ID for reuse, handles all errors safely.

import requests, sys, json, os, time

# -------- CONFIG (edit if needed) --------
BOT_TOKEN = "7391593372:AAFhLbgDhxgNmMZwlLIzB1VuxNnxykV83XQ"
CHANNEL_ID = "-1002162858751"     # private/public numeric id
CHANNEL_USERNAME = "@unsely"      # optional fallback username
USER_FILE = "user_id.json"        # where to store/reuse user id
# -----------------------------------------
...
def get_user_id():
    """Load saved user id or ask once."""
...
def get_chat_member(chat, user):
    """Call Telegram API getChatMember safely."""
    try:
...
def check_membership(user_id):
    """Return dict {joined, status, error}."""
    chats = [CHANNEL_ID, CHANNEL_USERNAME]
    for chat in chats:
        data = get_chat_member(chat, user_id)
        if not data.get("ok"):
            err = data.get("description", "Unknown error")
            # Skip invalid participant errors and try next chat
...
def main():
    print(f"{cyan}Telegram Membership Checker{reset}")
    uid = get_user_id()
    print(f"{cyan}Checking membership for User ID: {uid}...{reset}")
    time.sleep(0.5)
    ...
    if res["error"]:
        ...
        return
    if res["joined"]:
        print(f"{green}[✔] User is a member! Access granted.{reset}")
    else:
        print(f"{red}[✖] User not in channel! Access denied.{reset}")
